...
# Declare Verifiable Obstacle Detection variables
VOD=verifiable_obstacle_detection
VOD_DIR_SRC=$SRC_DIR/$VOD
VOD_DIR_BUILD_DB=$BUILD_DIR/$VOD/$(echo ${DB_TYPE,,})
VOD_DIR_BUILD_RL=$BUILD_DIR/$VOD/$(echo ${RL_TYPE,,})
VOD_FILE_CMAKE_TOOLCHAIN=$PROJECT_DIR/cmake/$VOD/arm64.toolchain.cmake

echo "[INFO]: Setting up Verifiable Obstacle Detection for $ARCH architecture..."
...
if [ $LOCAL -eq 1 ]; then
	...
	RETURN=$?
	if [ $RETURN -ne 0 ]; then
		echo "[ERROR]: Setup for Verifiable Obstacle Detection failed. Quit."
		exit $RETURN
	fi
...
# Create Verifiable Obstacle Detection projects
echo "[INFO]: Creating Verifiable Obstacle Detection projects..."
echo "[INFO]: Creating Verifiable Obstacle Detection debug project..."
if [ ! -d "$VOD_DIR_BUILD_DB" ]; then
	mkdir -p "$VOD_DIR_BUILD_DB"
	cd "$VOD_DIR_BUILD_DB"

	if [ "$ARCH" = "arm64" ]; then
		cmake -G "Eclipse CDT4 - Unix Makefiles" -DCMAKE_BUILD_TYPE=$DB_TYPE -DCMAKE_ECLIPSE_MAKE_ARGUMENTS=-j$CORES -DCMAKE_ECLIPSE_VERSION=$ECLIPSE -DCMAKE_TOOLCHAIN_FILE="$VOD_FILE_CMAKE_TOOLCHAIN" -DARCH=$ARCH -DLOCAL=1 "$VOD_DIR_SRC"
	else
		cmake -G "Eclipse CDT4 - Unix Makefiles" -DCMAKE_BUILD_TYPE=$DB_TYPE -DCMAKE_ECLIPSE_MAKE_ARGUMENTS=-j$CORES -DCMAKE_ECLIPSE_VERSION=$ECLIPSE -DARCH=$ARCH -DLOCAL=$LOCAL "$VOD_DIR_SRC"
	fi
	...
	echo "[INFO]: Verifiable Obstacle Detection debug project exists. Skip."
fi
echo "[INFO]: Creating Verifiable Obstacle Detection release project..."
if [ ! -d "$VOD_DIR_BUILD_RL" ]; then
	mkdir -p "$VOD_DIR_BUILD_RL"
	cd "$VOD_DIR_BUILD_RL"

	if [ "$ARCH" = "arm64" ]; then
		cmake -G "Eclipse CDT4 - Unix Makefiles" -DCMAKE_BUILD_TYPE=$RL_TYPE -DCMAKE_ECLIPSE_MAKE_ARGUMENTS=-j$CORES -DCMAKE_ECLIPSE_VERSION=$ECLIPSE -DCMAKE_TOOLCHAIN_FILE="$VOD_FILE_CMAKE_TOOLCHAIN" -DARCH=$ARCH -DLOCAL=1 "$VOD_DIR_SRC"
	else
		cmake -G "Eclipse CDT4 - Unix Makefiles" -DCMAKE_BUILD_TYPE=$RL_TYPE -DCMAKE_ECLIPSE_MAKE_ARGUMENTS=-j$CORES -DCMAKE_ECLIPSE_VERSION=$ECLIPSE -DARCH=$ARCH -DLOCAL=$LOCAL "$VOD_DIR_SRC"
	fi
	...
	echo "[INFO]: Verifiable Obstacle Detection release project exists. Skip."
fi

echo "[INFO]: Setup for Verifiable Obstacle Detection completed for $ARCH architecture."
